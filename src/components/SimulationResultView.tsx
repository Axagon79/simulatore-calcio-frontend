...
interface SimulationData {
  ...
  sign: string;
  algo_name: string;
  statistiche_partita: Record<string, [string | number, string | number]>;
  cronaca_live: MatchEvent[];
  report_scommesse: {
...
const SimulationResultView: React.FC<Props> = ({ data, onOpenBettingDetails, onOpenAIExplanation }) => {
  ...
  const calculateWidth = (home: any, away: any) => {
    const h = parseFloat(home.toString().replace('%', ''));
    const a = parseFloat(away.toString().replace('%', ''));
    const total = h + a;
  ...
  return (
    <div className="w-full max-w-5xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      {/* HEADER: RISULTATO E AFFIDABILITÀ SCIENTIFICA */}
      <div className="relative overflow-hidden bg-slate-900/50 border border-slate-700 p-8 rounded-3xl text-center shadow-2xl">
        ...
        <p className="text-cyan-400 font-bold tracking-widest uppercase text-sm mb-4">Simulazione Completata • {data.algo_name}</p>
        
        <div className="flex items-center justify-center gap-12 mb-6">
          <div className="text-right">
            <h2 className="text-2xl font-bold text-white">Cagliari</h2>
          ...
          <div className="text-left">
            <h2 className="text-2xl font-bold text-white">Milan</h2>
            <p className="text-slate-400 text-sm">OSPITE</p>
          </div>
        </div>
        ...
        <div className="flex justify-center gap-4">
          ...
          <div className="bg-slate-800/80 px-4 py-2 rounded-full border border-slate-600 flex items-center gap-2">
            ...
            <span className="text-slate-200 text-sm font-medium">Affidabilità: <b className="text-emerald-400">{data.report_scommesse.Analisi_Profonda.Confidence_Globale}</b></span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-slate-900/50 border border-slate-700 p-6 rounded-3xl">
          ...
          <div className="space-y-6">
            {Object.entries(data.statistiche_partita).map(([label, [valH, valA]]) => (
        ...
        <div className="bg-slate-900/50 border border-slate-700 p-6 rounded-3xl">
          ...
          <div className="relative space-y-8 before:absolute before:left-[17px] before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-700">
            {data.cronaca_live.map((event, idx) => (
